...
### Added
- `docs/token/wtag-bonding-curve.mdx` — Full developer reference for `wTAGBondingCurve`: linear bonding curve for wTAG price discovery. Covers `buy(uint256 tokenAmount, uint256 maxCost)`, `sell(uint256 tokenAmount, uint256 minReturn)`, `getBuyQuote(uint256)`, `getSellQuote(uint256)`, `currentPrice()` view functions; `updateParams(uint256, uint256, uint256)`, `withdrawFees()`, `pause()`/`unpause()` admin functions; bonding curve integral math (`P₀·N/PREC + slope·N·(2S±N)/(2·PREC²)`); 4 events (`Buy`, `Sell`, `FeesWithdrawn`, `ParamsUpdated`); 12 custom errors; `PRECISION`, `MAX_FEE_BPS`, `MAX_SLOPE` constants; `ReentrancyGuard` + `Pausable` + `Ownable` + CEI security model; slippage protection (`maxCost`/`minReturn`); 664-line test suite (constructor, buy/sell, admin, pause, slippage, fee accrual, integration, fuzz). GitHub Wiki: [`wiki/wTAGBondingCurve.md`](../wiki/wTAGBondingCurve.md). Covers task [3314e3e9-a2d3-8185] ([tagit-contracts PR #11](https://github.com/TAG-IT-NETWORK/tagit-contracts/pull/11)).
